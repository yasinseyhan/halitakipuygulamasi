...
const Received = ({ navigation }) => {
  ...
  const orderStatuses = [
    "Teslim Alınacak",
    "Teslim Alındı", // Mevcut durum
    "Yıkamada", // Sonraki durum
    "Hazır",
    "Teslim Edilecek",
    "Teslim Edildi",
    "İptal Edildi",
  ...
  useEffect(() => {
    // Sadece "Teslim Alındı" durumundaki siparişleri dinle
    const q = query(
      collection(firestore, "orders"),
      where("status", "==", "Teslim Alındı"), // Ana filtreleme burada!
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error(
          "Teslim alınmış siparişler çekilirken hata oluştu: ",
          error
        );
        Alert.alert(
          "Hata",
          "Teslim alınmış siparişler yüklenirken bir sorun oluştu."
        );
        setLoading(false);
      }
    );
  ...
  const renderOrderItem = ({ item }) => (
    <View style={styles.orderCard}>
      ...
      {/* Sadece "Teslim Alındı" durumundaki siparişler için butonu göster */}
      {item.status === "Teslim Alındı" && (
        <TouchableOpacity
          style={styles.actionButton}
          onPress={() => updateOrderStatus(item.id, item.status)}
        >
          <Text style={styles.actionButtonText}>Yıkamaya Gönder</Text>
          <Ionicons
    ...
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.loadingText}>
          Teslim alınmış siparişler yükleniyor...
        </Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      {orders.length === 0 ? (
        <Text style={styles.noOrdersText}>
          Henüz teslim alınmış sipariş bulunmamaktadır.
        </Text>
      ) : (
        <FlatList
          data={orders}
